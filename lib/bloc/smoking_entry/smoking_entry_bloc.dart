  ...
  Stream<SmokingEntryState> mapEventToState(SmokingEntryEvent event) async* {
    if (event is FetchSmokingEntry) {
      ...
      try {
        ...
        final dateGroupedEntries =
            KtList.from(entries).filter((entry) => entry.hasSmoked == true).groupBy((entry) => DateFormat('yyyy-MM-dd').format(entry.datetime));

        final nonSmokedDates = KtList.from(entries).groupBy((entry) => DateFormat('yyyy-MM-dd').format(entry.datetime)).filter((date) {
          return date.value.filter((entry) => entry.hasSmoked).count() == 0;
        });

        print(nonSmokedDates);
